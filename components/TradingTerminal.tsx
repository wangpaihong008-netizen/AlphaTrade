...
// 1. Custom Candlestick Shape for Recharts
  ...
  return (
    <div className="flex flex-col h-full bg-[#0b101a] border-l border-slate-800 w-64 hidden xl:flex">
      ...
      <div className={`py-2 px-3 text-lg font-bold border-y border-slate-800 my-1 flex items-center justify-center ${isUp ? 'text-green-500 bg-green-500/5' : 'text-red-500 bg-red-500/5'}`}>
         {price.toFixed(2)} 
         {isUp ? <TrendingUp className="w-4 h-4 ml-2" /> : <TrendingDown className="w-4 h-4 ml-2" />}
      </div>
...
const TradingTerminal: React.FC<TradingTerminalProps> = ({ 
  ...
  const [gridConfig, setGridConfig] = useState<Partial<GridStrategyConfig>>({
    investment: 1000,
    leverage: 3,
    grids: 20
  });
    ...
    <div className="h-full flex flex-col md:flex-row overflow-hidden bg-[#0b101a]">
        ...
        <div className="h-14 flex justify-between items-center px-4 border-b border-slate-800 bg-slate-950 shrink-0">
           <div className="relative z-20">
             <button 
               ...
               className="flex items-center space-x-2 hover:bg-slate-900 px-2 py-1 rounded transition-colors group"
             >
                 ...
                 {coins?.map(c => (
                   <div key={c.symbol} onClick={() => { onCoinChange(c); setShowCoinMenu(false); }} className="px-4 py-3 hover:bg-slate-800 cursor-pointer flex justify-between items-center border-b border-slate-800/50">
                     ...
                     <div className="text-right">
                       <div className="text-xs text-slate-400">${c.price}</div>
                       <div className={`text-[10px] ${c.change24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>{c.change24h}%</div>
                     </div>
                   </div>
                 ))}
               </div>
           ...
           <div className="flex space-x-6 items-center">
              <div>
                <span className={`text-lg font-mono font-bold ${activeCoin.change24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                    ${activeCoin.price.toLocaleString()}
                </span>
                <span className="text-xs text-slate-500 ml-2">Mark</span>
              </div>
              <div className="hidden md:flex space-x-1">
                 {['1m', '5m', '15m', '1H', '4H'].map(t => (
                    <button key={t} className={`px-2 py-1 text-xs font-bold rounded hover:text-white ${
                        (futuresStyle === 'SCALPING' && t === '5m') || (futuresStyle !== 'SCALPING' && t === '15m') 
                        ? 'bg-slate-800 text-trade-accent' : 'text-slate-500'
                    }`}>{t}</button>
                 ))}
              </div>
           </div>
           ...
           {candleData.length > 0 ? (
               <ResponsiveContainer width="100%" height="100%">
                 <ComposedChart data={candleData} margin={{ top: 20, right: 60, left: 10, bottom: 20 }}>
                   <XAxis 
                     dataKey="time" 
                     tickFormatter={(t) => new Date(t).getHours() + ':' + new Date(t).getMinutes()} 
                     minTickGap={40}
                   ...
                   {futuresAdvice && (
                     <>
                       ...
                       <ReferenceLine y={futuresAdvice.takeProfit} stroke="#10b981" strokeWidth={1} label={{value: 'TP', position: 'insideRight', fill: '#10b981', fontSize: 10}} />
                     </>
                   )}
                 </ComposedChart>
               </ResponsiveContainer>
           ) : (
             <div className="flex items-center justify-center h-full text-slate-600">Loading Chart Data...</div>
           )}
           
           <div className="absolute top-4 left-4 flex flex-col space-y-2 pointer-events-none">
              <div className="bg-slate-900/90 backdrop-blur px-3 py-2 rounded border border-slate-700/50 text-xs shadow-lg">
                 <span className="text-slate-400 block uppercase text-[10px]">24h Volume</span>
                 <span className="text-slate-200 font-mono">{activeCoin.volume24h}</span>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-80 lg:w-96 bg-slate-950 border-l border-slate-800 flex flex-col z-10 shadow-2xl shrink-0">
         <div className="flex border-b border-slate-800 bg-slate-950">
            <button onClick={() => setMode('GRID')} className={`flex-1 py-4 text-sm font-bold border-b-2 transition-all ${mode === 'GRID' ? 'border-trade-accent text-trade-accent bg-slate-900' : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-900/50'}`}>网格策略</button>
            <button onClick={() => setMode('FUTURES')} className={`flex-1 py-4 text-sm font-bold border-b-2 transition-all ${mode === 'FUTURES' ? 'border-trade-accent text-trade-accent bg-slate-900' : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-900/50'}`}>合约交易</button>
         </div>
               ...
               <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                  <div>
                     ...
                     <div className="grid grid-cols-3 gap-2">
                        {['LONG', 'NEUTRAL', 'SHORT'].map(d => (
                           <button 
                              key={d}
                              onClick={() => setGridDirection(d as any)}
                              className={`py-2.5 text-xs font-bold rounded-lg border transition-all ${gridDirection === d ? 'bg-slate-800 border-slate-500 text-white' : 'border-slate-800 bg-slate-900 text-slate-500 hover:border-slate-600'}`}
                           >
                              {d === 'NEUTRAL' ? '中性' : d === 'LONG' ? '做多' : '做空'}
                           </button>
                        ))}
                     </div>
                  </div>
                  ...
                  <div className="p-0.5 rounded-xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500">
                      <div className="bg-slate-900 rounded-[10px] p-4 relative overflow-hidden">
                         <div className="relative z-10">
                            <h4 className="text-white font-bold text-sm flex items-center mb-1"><Zap className="w-4 h-4 mr-1 text-yellow-400" /> AI 智能策略 (Auto)</h4>
                            <p className="text-[10px] text-slate-400 mb-3">基于 {promptConfig?.mode} 模式分析支撑压力位。</p>
                            <button onClick={handleAiOptimize} disabled={loadingAI} className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg flex items-center justify-center transition-all shadow-lg shadow-indigo-500/20">
                               {loadingAI ? <RefreshCw className="w-3 h-3 animate-spin mr-2" /> : <Settings2 className="w-3 h-3 mr-2" />}
                               {loadingAI ? '分析中...' : '生成最优策略'}
                            </button>
                         </div>
                      </div>
                  </div>
                  ...
                  <div className="space-y-4 bg-slate-900/50 p-4 rounded-xl border border-slate-800">
                     <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-500 font-bold uppercase">价格区间</span>
                        <div className="flex space-x-2">
                           <input type="number" className="w-20 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-right text-slate-200" value={gridConfig.lowerPrice} readOnly />
                           <span className="text-slate-600">-</span>
                           <input type="number" className="w-20 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-right text-slate-200" value={gridConfig.upperPrice} readOnly />
                        </div>
                     </div>
                     <div className="flex justify-between items-center text-xs">
                         <span className="text-slate-500 font-bold uppercase">投入金额</span>
                         <input type="number" className="w-24 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-right text-white font-bold" value={gridConfig.investment} onChange={e => setGridConfig({...gridConfig, investment: parseFloat(e.target.value)})} />
                     </div>
                  </div>

                  <button onClick={executeGrid} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-lg shadow-emerald-900/20 flex items-center justify-center">
                     <Play className="w-5 h-5 mr-2" /> 启动网格
                  </button>
               </div>
            ) : (
               <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                  <div className="bg-slate-900 p-1 rounded-xl grid grid-cols-2 gap-1 border border-slate-800">
                     <button onClick={() => setFuturesStyle('STANDARD')} className={`py-2 text-[10px] font-bold rounded-lg transition-all ${futuresStyle === 'STANDARD' ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}>标准模式</button>
                     <button onClick={() => setFuturesStyle('SCALPING')} className={`py-2 text-[10px] font-bold rounded-lg transition-all flex items-center justify-center ${futuresStyle === 'SCALPING' ? 'bg-purple-600 text-white shadow-lg shadow-purple-900/30' : 'text-slate-500 hover:text-slate-300'}`}>
                        <Flame className="w-3 h-3 mr-1" /> 拔头皮 (Scalping)
                     </button>
                  </div>
                     ...
                     <div className="bg-slate-900 rounded-[10px] p-4">
                        <div className="flex justify-between items-center mb-4">
                           <h4 className={`text-xs font-bold flex items-center ${futuresStyle === 'SCALPING' ? 'text-purple-400' : 'text-blue-400'}`}>
                              {futuresStyle === 'SCALPING' ? <Skull className="w-4 h-4 mr-1"/> : <Zap className="w-4 h-4 mr-1"/>}
                              {futuresStyle === 'SCALPING' ? 'AI 高频信号' : 'AI 趋势建议'}
                           </h4>
                           <button onClick={handleFuturesAdvice} disabled={loadingAI} className="text-[10px] underline text-slate-500 hover:text-white flex items-center">
                              {loadingAI ? <RefreshCw className="w-3 h-3 animate-spin mr-1"/> : '刷新信号'}
                           </button>
                        </div>
                           ...
                           <div className="space-y-3">
                              <div className="flex items-center justify-between bg-slate-950 p-3 rounded-lg border border-slate-800">
                                 ...
                                 <div className="text-right">
                                    ...
                                    <span className="font-mono text-white text-sm">{futuresAdvice.entryPrice}</span>
                                 </div>
                              </div>
                              <p className="text-[10px] text-slate-400 leading-tight pt-2 border-t border-slate-800/50 italic">"{futuresAdvice.reasoning}"</p>
                              <button onClick={() => onOpenPosition({ id: Math.random().toString(), symbol: activeCoin.symbol, side: futuresAdvice.direction, entryPrice: activeCoin.price, markPrice: activeCoin.price, size: 100, leverage: futuresAdvice.leverage, pnl: 0, pnlPercent: 0 })} className={`w-full py-3 mt-1 rounded-lg text-xs font-bold text-white flex items-center justify-center transition-all hover:brightness-110 shadow-lg ${futuresStyle === 'SCALPING' ? 'bg-purple-600 shadow-purple-900/20' : 'bg-blue-600 shadow-blue-900/20'}`}>
                                 <ArrowRightCircle className="w-3 h-3 mr-1" /> 一键跟单 (x{futuresAdvice.leverage})
                              </button>
                           </div>
                        ) : (
                           <div className="text-center py-6 text-xs text-slate-500 border-2 border-dashed border-slate-800 rounded-lg">点击刷新获取信号</div>
                        )}
                     </div>
                  </div>
                  ...
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                     <div className="flex justify-between text-xs font-bold text-slate-500 mb-4">
                        <span>杠杆倍数</span>
                        <span className={`px-2 py-0.5 rounded text-white ${futuresLeverage > 20 ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>{futuresLeverage}x</span>
                     </div>
